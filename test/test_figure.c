...
TEST(figure_plots_independent)
{
    ...
    plm_plot_add_line(a, ax, ay, 3, (plm_line_style){PLM_BLUE, 1.f, NULL});
...
TEST(figure_render_smoke)
{
    ...
    for (int r = 0; r < 2; r++) {
        for (int c = 0; c < 2; c++) {
            ...
            plm_plot_add_line(p, x, y, 5, (plm_line_style){PLM_BLUE, 1.f, NULL});
        }
    }
...
TEST(figure_reset_clears)
{
    ...
    float x[] = { 1.f };
    float y[] = { 2.f };
    plm_plot_add_line(p, x, y, 1, (plm_line_style){PLM_BLUE, 1.f, NULL});
